...
class QemuManager {
  constructor(options) {
    ...
    this.networks_config_folder =
      options?.networks_config_folder || "/etc/libvirt/qemu/networks";
  }
  ...
  async getServers() {
    ...
    for (var serverConfigFilePath of fileListFiltered) {
      const fileContentXml = fs.readFileSync(
        `/etc/libvirt/qemu/${serverConfigFilePath}`,
        "utf-8"
      );

      if (fileContentXml.includes("uuid")) {
