...
class QemuMemballoon {
    ...
    get model() {
        return this.#xml[0].$.model;
    }

    set model(value) {
        this.#xml[0].$.model = value;
        this.#populateClass();
    }
    
    //QEMU MEMBALLOON BUS GETTER, SETTER
    get bus() {
        return this.#xml[0].address[0].$.bus;
    }

    set bus(value) {
        this.#xml[0].address[0].$.bus = value;
        this.#populateClass();
    }

    //QEMU MEMBALLOON SLOT GETTER, SETTER
    get slot() {
        return this.#xml[0].address[0].$.slot;
    }

    set slot(value) {
        this.#xml[0].address[0].$.slot = value;
        this.#populateClass();
    }
